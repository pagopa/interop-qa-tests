...
const run = async () => {
  ...
  try {
    if (
      !process.env.DOMAIN_TOPIC_PREFIX &&
      !process.env.DEBEZIUM_OFFSETS_TOPIC
    ) {
      console.error(
    ...
    if (kafkaTopics?.length) {
      ...
      kafkaTopics.forEach((topic) => {
        ...
        if (
          ...
          sanitizedPrefix.some((sp) => topic.startsWith(sp))
        ) {
        ...
        if (
          !pushed &&
          sanitizedExactMatch.length &&
          sanitizedExactMatch.some((sp) => topic === sp)
        ) {
          topicsToDelete.push(topic);
          pushed = true;
        }
