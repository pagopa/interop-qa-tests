...
export function getTopicRequiredOptionKey(optionKey) {
  return getTopicRequiredOptions().find((ro) => ro.expectedLabel === optionKey)
    .targetLabel;
}

export function isTopicDefaultConfig(configKey) {
  const defaultTopicConfiguration = getTopicDefaultConfiguration();

  return (
    defaultTopicConfiguration.findIndex((ce) => ce.name === configKey) >= 0
  );
}
